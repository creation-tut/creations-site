...
foreach ($list as $key => $data) {
    ...
    // copy images
    echo "  copy teiki.jpg...";
    if (copy(IMAGE_DIR_NAME.'/'.$data['imgFileName'], $workDir.'teiki.jpg')) {
        echo "ok.\n  copy teiki_s.jpg...";
    } else {
        exit("Error: failed");
    }

    if (copy(IMAGE_DIR_NAME.'/'.$data['thmFileName'], $workDir.'teiki_s.jpg')) {
        echo "ok.\n";
    } else {
        exit("Error: failed");
    }
    if ($theme['isRemake']) {
        echo "  copy teiki2.jpg...";
        if (copy(IMAGE_DIR_NAME.'/'.$data['origFileName'], $workDir.'teiki2.jpg')) {
            echo "ok.\n";
        } else {
            exit("Error: filed");
        }
